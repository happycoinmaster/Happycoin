...
{
    // HappyCoin: check prefix
    if(uri.scheme() != QString("HappyCoin"))
        return false;
...
{
    // Convert HappyCoin:// to HappyCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("HappyCoin://"))
    {
        uri.replace(0, 10, "HappyCoin:");
    }
...
void openConfigfile()
{
    ...
    /* Open HappyCoin.conf with the associated application */
